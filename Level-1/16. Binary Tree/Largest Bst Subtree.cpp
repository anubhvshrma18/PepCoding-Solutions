...
struct Pair
{
    ...
    Pair(int mn, int mx, bool isBST, int sz, Node *t)
    {
        this->mn = mn;
        this->mx = mx;
        this->isBST = isBST;
...
// This method is efficient and good but in some edge cases it may give error
...
//        return false;
//    }
//    return isBSTHelper(root->left,mn,root->data-1) && isBSTHelper(root->right,root->data+1,mx);
//}
//bool isBST(Node* root){
//
...
// 3rd method
// this is most efficient and best method. In order Traversal of Tree
...
Pair largestBSTSubtree(Node *root)
{
    if (root == nullptr)
    {
        Pair p(INT_MAX, INT_MIN, true, 0, nullptr);
        return p;
    }
    Pair lp = largestBSTSubtree(root->left);
    Pair rp = largestBSTSubtree(root->right);

    bool isBST = (lp.mx < root->data && rp.mn > root->data) && lp.isBST && rp.isBST;
    int mini = min(min(lp.mn, rp.mn), root->data);
    int maxi = max(max(lp.mx, rp.mx), root->data);
    ...
    Pair p(mini, maxi, isBST, size, node);
    return p;
}
